...
Create `.env` beside `app.py` with Wazuh API, indexer, and Ollama settings. `client.py` loads this file at import time.
...
- `TOOLS` registry plus JSON schemas exposed to Ollama tool calling
- `run_agent()` loop: send messages to Ollama, execute requested tools, append tool results back into conversation, stop on final prose answer or step cap
...
4. `run_agent()` calls Ollama with tool schemas.
